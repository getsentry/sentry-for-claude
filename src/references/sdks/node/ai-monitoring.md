...
This generic `Sentry.init` path applies to the provider integrations below.
If Eve or Flue is detected, skip it and use that framework’s dedicated setup section
instead.
...
On Node.js, when `eve` is present, use Eve’s official Sentry instrumentation.
It exports Eve’s OpenTelemetry GenAI spans directly to Sentry, so do not also add the
Sentry Vercel AI integration — two span producers double-count tokens and cost.
See the
...
| Eve token or cost totals doubled | Remove the Sentry Vercel AI integration; Eve already emits the AI spans |
